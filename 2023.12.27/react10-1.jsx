...
function ReactTenDashOne() {
  ...
  const togglePack = (index) => {
    ...
    setPacks(newPacks); // srtPacks는 useState() 메서드의 1번째이다
    // 구조 분해 할당으로 '할당' 받았으며 1번째 (두번째)는 React에 의해 이미 만들어진 함수(메서드)이다.
  };
  const addPack = () => {
    ...
    <div>
      ...
      <div>
        {packs.map((isOpen, index) => (
          <div ket={index} style={{ cursor: 'pointer', backgroundColor: isOpen ? 'salmon' : 'white' }} onClick={() => togglePack(index)}>
            pack {index + 1}
          </div>
        ))}
      </div>
