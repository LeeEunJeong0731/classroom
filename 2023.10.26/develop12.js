 ...
 *  * 어떤 부분을 확인 해야 적절 하게 해당 로직을 이해할 수 있을까?
 * 
 */
...
function bundleFiles(options) {
  ...
  // 모듈의 재사용성을 고려하며, 유효성 검사를 위한 함수를 만들어 놓는 모듈을 사용함

// 초기화 (init)
// **options **
// 구조분해 할당 작성법을 사용하면 훨씬 간단해지지만, 어떻게 초기화 하는지를 확인 하기 위헤
// 상수(변수) 선언 방식으로 작성함.
  ...
  function replaceTagsInHtml(html, cssContent, jsContent) {
    ...
    //매세드 체어닝을 사용하여, replace 메서드를 연속적으로 호출함
    //메서드 체어닝 테크닉을 사용하지 않으면 , 아래와 같이 작성 해야 함
    // 변수 낭비가 심함
